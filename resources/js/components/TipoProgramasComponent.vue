                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="mb-3 col-sm-12">
                                    <label for="nombre" class="form-label">Nombre del Tipo de Programa</label>
                                    <input v-model="tipoPrograma.nombre" class="form-control" type="text" placeholder="Tipo de Programa" aria-label="default input example">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered border-primary">
                    ...
                    <tbody class="text-center">
                        <tr v-for="tipoPrograma in tipoProgramas" :key="tipoPrograma.id">
                            <th >{{tipoPrograma.id}}</th>
                            <td>{{tipoPrograma.nombre}}</td>
                            <td>
                                <button type="button" @click="update=true; openModal(tipoPrograma);" data-bs-toggle="modal" data-bs-target="#exampleModal" class="btn btn-outline-info btn-sm">
                            ...
                            <td>
                                <button type="button" @click="update=true; eliminarM(tipoPrograma.id);" class="btn btn-outline-danger btn-sm">
    ...
    export default 
    {
        ...
        methods:
        {
            async list()
            {
                ...
                {
                    this.cargando = true;
                    const res = await axios.get('/dashboard/tipo_programas_api');
                    this.tipoProgramas = res.data;
                    this.cargando = false;
                }
                catch(error)
                {
            ...
            async eliminar(id) 
            {
                try
                {   
                    const res = await axios.delete('/dashboard/tipo_programas_api/'+id)
                    .then(response=>
                    {
                        if(response.data==1)
                        {
            ...
            async save()
            {
                if(this.update)
                {
                    try
                    {
                        ...
                        for(let key in this.tipoPrograma)
                        {
                            fields.append(key,this.tipoPrograma[key]);
                        }
                        //.then(response=>{console.log(response.data)})
                        const res = await axios.put('/dashboard/tipo_programas_api/'+this.id, this.tipoPrograma)
                        .then(response=>{
                            if(response.data==1)
                            {
                ...
                else
                {
                    try
                    {
                        ...
                        //.then(response=>{console.log(response.data)})
                        const res = await axios.post('/dashboard/tipo_programas_api', fields)
                        .then(response=>
                        {
                            if(response.data==1)
                            {
            ...
            openModal(data={}) 
            {
                ...
                try
                {
                    if(this.update)
                    {
                        this.titleModal = "Modificar Tipo Programa";
                        this.id=data.id;
                        this.tipoPrograma.id = data.id;
                        this.tipoPrograma.nombre = data.nombre;
                    }
                    else
                    {
                        this.titleModal = "Agregar Tipo Programa";
                        this.id=0;
                        this.tipoPrograma.nombre = '';
                    }
                }
                catch(error)
                {
